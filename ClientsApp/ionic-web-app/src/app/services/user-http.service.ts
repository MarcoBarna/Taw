...
export class UserHttpService {
  ...
  private set_token(token) {
    ...
    const exp_date =
      decoded_token.iat * 1000 +
      Math.floor((decoded_token.exp - decoded_token.iat) * 1000 * 0.9);
    const now = new Date().getTime();
    if (exp_date - now > 0) {
      this.renew_clock_interval = exp_date - now;
      this.renew_clock = setInterval(() => {
  ...
  login(nick: string, password: string): Observable<any> {
    ...
    return this.http.get(/*this.url + */ 'users/login', options).pipe(
      tap(data => {
  ...
  public renew(): Observable<any> {
    ...
    if (!tk || tk.length < 1) {
      return throwError({
        error: { errormessage: 'No token found in local storage' }
      });
    }
